...
def create_predictions_df(predictions,images):
    ...
    if st.session_state.pred_df.empty:
        for i,img in enumerate(images):
            ...
            st.session_state.pred_df=pd.concat([st.session_state.pred_df,pd.DataFrame.from_records([row])],ignore_index=True)
...
def show_curr_image_classifier(images_list):
    ...
    if st.session_state.img_idx>=len(images_list):
        ...
        if st.session_state.pred_df.empty ==False:
            curr_pred=st.session_state.pred_df[st.session_state.pred_df["Filename"]==images_list[st.session_state.img_idx].name]
